...
Content Extraction:
- Process the Tamil input content and identify entities mentioned within it.
- Focus on entities related to historical spine treatments, observations, and medical concepts.
- Ensure proper translation of Tamil medical terms to English equivalents.
...
- PathophysiologicalPattern
e.g., imbalances like “Qi stagnation”, “cold-damp obstruction”
-Useful for modeling diagnostic frameworks in traditional systems
-DiagnosticMethod
e.g., tongue diagnosis, pulse reading, imaging
Helps connect observations to assessment techniques
-Biomarker
e.g., lab values, imaging results, pulse findings
Bridges modern and traditional diagnostics
-PractitionerRole
e.g., herbalist, acupuncturist, orthopedic specialist
For attributing decisions, methods, or interpretations
-AnatomicalStructure
e.g., spine, vertebrae, nervous pathways
Helps ground observations and treatments spatially
-HealthOutcome
e.g., restored mobility, pain reduction, improved sleep
Separate from TherapeuticOutcome, focuses on patient goals
-DosageForm
e.g., decoction, acupuncture session, topical paste
Adds specificity to TherapeuticApproach
-TemporalPhase
e.g., acute, subacute, chronic, post-therapy
Refines the time aspect of observation/intervention
...
-indicates
From PathophysiologicalPattern to TherapeuticApproach or ClinicalObservation
e.g., "Qi stagnation" indicates "acupuncture along liver meridian"
-measured_by
ClinicalObservation or Biomarker measured_by DiagnosticMethod
-administered_by
TherapeuticApproach administered_by PractitionerRole
-targets
TherapeuticApproach targets AnatomicalStructure or PathophysiologicalPattern
-progresses_to
ClinicalObservation or PathophysiologicalPattern progresses_to another
Models disease progression
-contraindicated_in
TherapeuticApproach contraindicated_in ContextualFactor (e.g., "heat therapy" in "febrile state")
-expressed_during
ClinicalObservation expressed_during TemporalPhase
-documented_by
Observation, Outcome, or Approach documented_by PractitionerRole
...
Node(id='sidha medicine', type='SourceText')
...
class KnowledgeGraphAgent(ChatAgent):
    ...
    def run(
        ...
        if response is None or response.msg is None:
            raise ValueError("Agent response is None. Check if the model is responding or input is too large.")
